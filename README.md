...
```bash
sam-api-example$ sam local invoke NetCodeWebAPIServerless --event events/event.json
```

The AWS SAM CLI can also emulate your application's API. Use the `sam local start-api` command to run the API locally on port 3000.

```bash
sam-api-example$ sam local start-api
sam-api-example$ curl http://localhost:3000/
```
...
```bash
sam-api-example$ sam logs -n NetCodeWebAPIServerless --stack-name sam-api-example --tail
```
...
```bash
sam-api-example$ dotnet test test/ServerlessAPI.Tests/ServerlessAPI.Tests.csproj 
```
...
```bash
sam delete --stack-name sam-api-example
```
